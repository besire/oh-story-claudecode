...
- **数据来源**：实时搜索 / 用户提供 / 内置知识 / CDP 抓取
- **数据时间**：榜单或资料对应的日期，未知则写「未知」
- **可信度**：高 / 中 / 低，并说明原因
- **限制说明**：样本数量、平台可见性、是否需要登录、短篇风口是否可能已经过期

不要把历史趋势写成实时结论；如果没有实时榜单，只能给「基于历史趋势的方向建议」。短篇风口变化快，超过 30 天的数据必须降一级可信度。
...
## 数据说明
- 数据来源：{实时搜索/用户提供/内置知识/CDP抓取}
- 数据时间：{日期或未知}
- 样本范围：{榜单名称/作品数量/平台范围}
- 可信度：{高/中/低} — {原因}
- 限制说明：{样本偏差/登录限制/算法影响/风口时效/无法验证项}
