...
| 模式 | 触发 | 输出 | 约束 |
|------|------|------|------|
| 只检测 | 「检查有没有AI味」「帮我看看」 | 只输出检测报告 | 不改写原文 |
| 小修 | 「去AI味」「润色一下」「自然一点」 | 保留原意和大部分句子，只做必要替换 | 不改剧情、不改人设、不重排段落 |
| 重写 | 「重写」「大改」「这段完全不像人写」 | 可重构句子和段落 | 先说明会明显改变表达方式，再输出新版 |

用户没有明确要求重写时，默认使用「小修」。如果文本涉及关键剧情、投稿稿件或用户强调保留风格，先询问处理强度。
...
| 场景 | 操作 |
|------|------|
| 用户贴一段文字说"太AI了" | 默认小修：检测 + 必要润色 |
| 用户说"帮我润色" | 先检测AI味，再按小修处理 |
| 用户说"检查下有没有AI味" | 只做检测，不做修改 |
| 用户说"重写/大改" | 进入重写模式，允许重构句子和段落 |
